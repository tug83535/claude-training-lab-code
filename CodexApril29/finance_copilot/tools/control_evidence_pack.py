...
EXCLUDE_FILENAMES = {"manifest.json", "README_EVIDENCE_PACK.txt"}
...
def _collect_files(input_dir: Path, output_dir: Path) -> list[Path]:
    ...
    for p in input_dir.rglob("*"):
        ...
        if p.name in EXCLUDE_FILENAMES:
            continue
        if p.suffix.lower() == ".zip" and p.parent.resolve() == output_dir.resolve():
            continue
        files.append(p)
    return files
...
def run(input_dir: Path, output_dir: Path, pack_name: str = "control_evidence_pack") -> Path:
    ...
    files = _collect_files(input_dir, output_dir)
    if not files:
        raise ValueError(f"No eligible files found in input directory: {input_dir}")
