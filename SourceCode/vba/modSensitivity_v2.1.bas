...
Public Sub RunSensitivityAnalysis()
    ...
    For d = 1 To driverCount
        ...
        ' For each scenario, estimate impact proportionally
        ' Simple linear sensitivity: change in driver -> proportional change in revenue
        Dim driverShare As Double
        driverShare = Abs(driverValues(d)) / Application.Max(baseRevenue, 1)
        If driverShare > 1 Then driverShare = 0.1  ' Cap for expense-type drivers
        ...
        For sc = 0 To 3
            ...
            impactEst = driverValues(d) * pctChange
